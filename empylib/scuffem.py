...
def make_spectral_files(wavelength, Material=None):
    ...
    with open("OmegaList.dat", "w") as f:
        for iw in range(len(wavelength)):
            f.write(f"{2 * _np.pi / wavelength[iw]:.6f}")
            if iw < len(wavelength) - 1:
                f.write("\n")

    lambda_mat = _np.insert(wavelength, 0, min(wavelength) * 0.9)
    lambda_mat = _np.append(lambda_mat, max(wavelength) * 1.1)
    w = 2 * _np.pi * c0 / lambda_mat * 1e6

    for mat_label, nk_raw in Material.items():
        if not isinstance(nk_raw, _np.ndarray):
        ...
        nk_data = _np.interp(lambda_mat, wavelength, nk_raw.astype(complex))
        eps = nk_data**2
...
def read_scatter_PFT(FileName):
    ...
    """
    df = _pd.read_csv(FileName, comment="#", sep=r"\s+", header=None)
    if df.empty:
        return {}
